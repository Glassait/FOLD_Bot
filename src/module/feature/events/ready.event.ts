...
export const event: BotEvent = {
    ...
    async execute(client: Client): Promise<void> {
        ...
        if (today.getDate() === 1) {
            ...
            listEmber.push(embedInt);

            let index = 0;

            if (playerClassement?.length > 0) {
                ...
                listEmber.push(embedScoreboard);
            } else {
                const embedNobody = new EmbedBuilder()
                    .setTitle('Tableau des scores')
                    .setDescription(
                        'Nous allons visualiser dans un premier temps le score des joueurs. \n(Plus de détails avec la commande `/trivia-statistics)`'
                    )
            ...
            if (winStrickPlayer?.length > 0) {
                ...
                listEmber.push(embedWinStrick);
            }

            if (stats.overall[monthStr]) {
