...
import { convertDateToMonthYearString, getPreviousMonthAsDate } from '../../../shared/utils/date.util';
import type { Logger } from '../../../shared/utils/logger';
import { getMaxFromArrayOfObject, getMinFromArrayOfObject } from '../../../shared/utils/math.util';
...
export class TriviaMonthModel {
    ...
    public async initialise(client: Client): Promise<void> {
        ...
        const winstreaks: Awaited<WinStreak | undefined>[] = await Promise.all(
            players.map(({ id }) => this.winStreakTable.getWinStreakFromDate(id, this.month))
        );
    ...
    private async createEmbed(): Promise<void> {
        ...
        if (this.playerClassement && this.playerClassement.length > 0) {
    ...
    private embedIntroduction(): void {
        this.listEmbed.push(
            new EmbedBuilder()
                .setTitle(transformToCode(`Résumé du mois de {}`, convertDateToMonthYearString(this.month)))
                        ...
                        this.playerClassement
                            ...
                            .reduce(
                                (text: string, [name, answer]) =>
                                    text + transformToCode(`${MEDAL[index++]} ${name} avec {} points\n`, answer[answer.length - 1].elo),
                                ''
                            ),
                });
                ...
                embed.addFields({
                    ...
                    value:
                        ...
                        this.playerClassement
                            ...
                            .reduce(
                                (text: string, [name, answer]) =>
                                    text + transformToCode(`${1 + index++} : ${name} avec {} points\n`, answer[answer.length - 1].elo),
                                ''
                            ),
                });
            }
    ...
    private embedQuickPlayer(): void {
        if (!this.playerClassement) {
            return;
        }

        this.playerClassement.sort(
            ([, aAnswers]: [string, TriviaAnswer[], WinStreak], [, bAnswers]: [string, TriviaAnswer[], WinStreak]) =>
                getMinFromArrayOfObject(aAnswers, 'answer_time') - getMinFromArrayOfObject(bAnswers, 'answer_time')
        );
            ...
            new EmbedBuilder()
                ...
                .setDescription(
                    transformToCode(
                        ...
                        name,
                        getMinFromArrayOfObject(answers, 'answer_time') / TimeEnum.SECONDE
                    )
                )
                .setColor(Colors.DarkGold)
    ...
    private embedSlowPlayer(): void {
        ...
        this.playerClassement.sort(
            ([, aAnswers]: [string, TriviaAnswer[], WinStreak], [, bAnswers]: [string, TriviaAnswer[], WinStreak]) =>
                getMaxFromArrayOfObject(bAnswers, 'answer_time') - getMaxFromArrayOfObject(aAnswers, 'answer_time')
        );
        const [name, answers] = this.playerClassement[0];
            ...
            new EmbedBuilder()
                ...
                .setDescription(
                    transformToCode(
                        ...
                        name,
                        getMaxFromArrayOfObject(answers, 'answer_time') / TimeEnum.SECONDE
                    )
                )
                .setColor(Colors.DarkGold)
