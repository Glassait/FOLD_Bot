 ...
 */
export function getMinFromArrayOfObject<T>(arr: T[], property: keyof T): number {
    const values: T[keyof T][] = arr.flatMap((obj: T) => obj[property]).filter((value: T[keyof T]): boolean => typeof value === 'number');
 ...
 */
export function getMaxFromArrayOfObject<T>(arr: T[], property: keyof T): number {
    const values: T[keyof T][] = arr.flatMap((obj: T) => obj[property]).filter((value: T[keyof T]): boolean => typeof value === 'number');
    if (!values.every((val: T[keyof T]): boolean => typeof val === 'number')) {
        throw new Error('Property values must be numbers.');
    }
