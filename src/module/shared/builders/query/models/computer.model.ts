 ...
 */
export function computeInsertInto(tableName: string, values: unknown[], columns?: string[], ignore?: boolean): string {
    return `INSERT${ignore ? ' IGNORE' : ''} INTO ${tableName} ${columns ? '(' + columns.join(', ') + ')' : ''} VALUES (${values.map((value: unknown): string => stringifyValue(value, true)).join(', ')})`;
}
 ...
 */
export function computeUpdate(tableName: string, values: unknown[], columns: string[], conditions?: Conditions): string {
    return `UPDATE ${tableName} SET ${reduceUpdate(values, columns)} ${conditions ? conditions.buildConditions() : ''}`;
}
 ...
 * // Here the condition is put as table of string, but is à instance of {@link Conditions}
 */
export function computeSelect(tableName: string, columns: string[], conditions?: Conditions): string {
 ...
 * // Here the condition is put as table of string, but is à instance of {@link Conditions}
 */
export function computeDelete(tableName: string, conditions?: Conditions): string {
 ...
 */
function reduceUpdate(values: unknown[], columns: string[]): string {
    return values.reduce(
        (set: string, value: unknown, index: number): string =>
            set + columns[index] + " = '" + stringifyValue(value) + (index === values.length - 1 ? "'" : "', "),
        ''
    );
}
 ...
 */
function stringifyValue(value: unknown, addQuote: boolean = false): string {
